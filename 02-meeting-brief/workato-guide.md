# Meeting Brief — Workato Implementation Guide
...
8. Build the primary recipe `02_meeting_brief_primary_recipe` with the trigger surface Callable recipe or Recipe Function invocation from a parent orchestrator.
...
- `normalize_run_context`: Normalize the Meeting Brief trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Meeting Brief business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, Meeting-system connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Meeting Brief, especially Enrich with Account Context. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Meeting Brief. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Meeting Brief. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Enrich with Account Context
3. Use an LLM connector or AI step to return strict JSON for AI Brief Generation
4. Use Gmail, Outlook, or SMTP connector for Deliver via Messaging
