# Opportunity Discovery — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Weekly |
...
7. Build the primary recipe `04_opportunity_discovery_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Opportunity Discovery trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Opportunity Discovery business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action.
- `assemble_business_context`: Join the intermediate context needed for Opportunity Discovery, especially Gather Activity & Pipeline Data, Identify Unmatched Accounts. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Opportunity Discovery. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Gather Activity & Pipeline Data
3. Use CRM connector action for Identify Unmatched Accounts
4. Use an LLM connector or AI step to return strict JSON for AI Signal Analysis
5. Use Gmail, Outlook, or SMTP connector for Notify via Messaging
...
- `ai_signal_analysis`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent evaluates each flagged account's activity patterns, contact seniority, and engagement intensity to score opportunity likelihood and recommend next steps.
