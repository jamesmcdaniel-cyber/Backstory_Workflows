# Forecast Coach — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Weekly (Monday) |
- Recommended Workato trigger surface: Scheduler by Workato
- Delivery targets: Email delivery
...
8. Build the primary recipe `05_forecast_coach_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Forecast Coach trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Forecast Coach business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Forecast Coach, especially Pull Team Pipeline, Compile Leader Reports. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Forecast Coach. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Forecast Coach. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Pull Team Pipeline
3. Use an LLM connector or AI step to return strict JSON for AI Deal Health Analysis
4. Use Backstory custom connector action for Compile Leader Reports
5. Use Gmail, Outlook, or SMTP connector for Deliver via Email
...
- `ai_deal_health_analysis`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent evaluates each deal across multiple dimensions (engagement, momentum, stakeholder mapping, competitive signals) and generates coaching-ready insights.
