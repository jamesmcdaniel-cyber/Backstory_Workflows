# Executive Inbox — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule |
- Recommended Workato trigger surface: Scheduler by Workato
- Delivery targets: Slack channel or direct message, Microsoft Teams channel or chat, Email delivery, Jira issue or project item, Asana task, Webhook, queue, database, or downstream workflow sink
...
8. Build the primary recipe `06_executive_inbox_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Executive Inbox trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Executive Inbox business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Executive Inbox, especially Identify Customer Emails, Enrich with Account Context, Await & Follow Up. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Executive Inbox. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Executive Inbox. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use email connector action for Identify Customer Emails
3. Use Backstory custom connector action for Enrich with Account Context
4. Use an LLM connector or AI step to return strict JSON for AI Triage & Classification
5. Use database, queue, webhook, or downstream app connector for Route to Channels
6. Use Backstory custom connector action for Await & Follow Up
...
- `ai_triage_classification`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent (via agent, structured output parser, and chain) analyzes email content plus account context to assign urgency, category, and routing recommendation.
