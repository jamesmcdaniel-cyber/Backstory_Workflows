# Churn Risk Scorecard — Workato Implementation Guide
...
7. Build the primary recipe `07_churn_risk_scorecard_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Churn Risk Scorecard trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Churn Risk Scorecard business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for Churn Risk Scorecard, especially Fetch Active Accounts, Enrich with Engagement Data, Compile Scorecard. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Churn Risk Scorecard. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use CRM connector action for Fetch Active Accounts
3. Use Backstory custom connector action for Enrich with Engagement Data
4. Use an LLM connector or AI step to return strict JSON for AI Risk Scoring
5. Use Backstory custom connector action for Compile Scorecard
6. Use Slack connector for Deliver to CS Managers
