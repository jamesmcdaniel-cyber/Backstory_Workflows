# Onboarding Pulse — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Daily 8:00 AM |
...
7. Build the primary recipe `09_onboarding_pulse_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Onboarding Pulse trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Onboarding Pulse business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for Onboarding Pulse, especially Find New Customers, Check Post-Sale Engagement. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Onboarding Pulse. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use CRM connector action for Find New Customers
3. Use Backstory custom connector action for Check Post-Sale Engagement
4. Use an LLM connector or AI step to return strict JSON for AI Engagement Assessment
5. Use Slack connector for Alert on Silent Accounts
...
- `ai_engagement_assessment`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent evaluates whether engagement is on track, at risk, or dark. Generates re-engagement recommendations for at-risk accounts.
