# Deal Hygiene Audit — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Weekly Monday 7:30 AM |
...
7. Build the primary recipe `11_deal_hygiene_audit_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Deal Hygiene Audit trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Deal Hygiene Audit business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for Deal Hygiene Audit, especially Pull Open Pipeline, Check Deal Engagement. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Deal Hygiene Audit. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use CRM connector action for Pull Open Pipeline
3. Use Backstory custom connector action for Check Deal Engagement
4. Use an LLM connector or AI step to return strict JSON for AI Hygiene Assessment
5. Use Slack connector for Deliver Action Lists
...
- `ai_hygiene_assessment`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent identifies hygiene issues per deal (stale, single-threaded, no exec, past close date) and prioritizes by deal value and stage.
