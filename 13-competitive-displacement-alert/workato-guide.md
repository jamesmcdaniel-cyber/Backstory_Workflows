# Competitive Displacement Alert — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Daily 7:00 AM |
...
7. Build the primary recipe `13_competitive_displacement_alert_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Competitive Displacement Alert trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Competitive Displacement Alert business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for Competitive Displacement Alert, especially Scan Engagement Drops, Check Competitor Signals. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Competitive Displacement Alert. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Scan Engagement Drops
3. Use CRM connector action for Check Competitor Signals
4. Use an LLM connector or AI step to return strict JSON for AI Displacement Assessment
5. Use Slack connector for Alert Account Team
...
- `ai_displacement_assessment`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent correlates engagement drops with competitor signals, assigns a displacement risk level, and generates a defensive action plan.
