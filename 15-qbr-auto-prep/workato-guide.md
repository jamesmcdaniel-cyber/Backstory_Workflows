# QBR Auto-Prep — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Weekly (configurable to quarterly cadence) |
- Recommended Workato trigger surface: Scheduler by Workato
- Delivery targets: Slack channel or direct message, Microsoft Teams channel or chat, Email delivery, Calendar event or task
...
7. Build the primary recipe `15_qbr_auto_prep_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the QBR Auto-Prep trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary QBR Auto-Prep business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for QBR Auto-Prep, especially Find Upcoming QBRs, Pull Quarterly Engagement. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for QBR Auto-Prep. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use calendar connector action for Find Upcoming QBRs
3. Use Backstory custom connector action for Pull Quarterly Engagement
4. Use an LLM connector or AI step to return strict JSON for AI QBR Document Generation
5. Use Slack connector for Deliver Prep Materials
...
- `ai_qbr_document_generation`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent produces a structured QBR prep document with executive summary, quarter-over-quarter trends, key wins, risk areas, and recommended talking points.
