# Executive Sponsor Tracker — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule — Daily 7:30 AM |
...
7. Build the primary recipe `16_executive_sponsor_tracker_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Executive Sponsor Tracker trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Executive Sponsor Tracker business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for Executive Sponsor Tracker, especially Find Strategic Deals, Check Executive Engagement. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Executive Sponsor Tracker. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use CRM connector action for Find Strategic Deals
3. Use Backstory custom connector action for Check Executive Engagement
4. Use an LLM connector or AI step to return strict JSON for AI Risk & Re-engagement
5. Use Slack connector for Alert Deal Owners
...
- `ai_risk_re_engagement`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent evaluates the impact of sponsor silence on deal health and generates specific re-engagement tactics per deal.
