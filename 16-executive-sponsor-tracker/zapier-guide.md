# Executive Sponsor Tracker — Zapier Implementation Guide
...
- `16_executive_sponsor_tracker_primary_zap` — Trigger: Schedule by Zapier. Native steps: Native app step: Find Strategic Deals -> Backstory custom Zapier app action: check_executive_engagement -> AI step backed by a native LLM app returning strict JSON for AI Risk & Re-engagement -> Native delivery step: Slack app action for Alert Deal Owners. Notes: Keep the primary Zap focused on normalization, source access, and structured synthesis. Route secondary fan-out through additional native Zaps when template restrictions or retry requirements demand decomposition.
- `16_executive_sponsor_tracker_delivery_or_followup_zap` — Trigger: Zapier Tables state change, Interfaces action, or an upstream custom app event. Native steps: Read delivery_payload and target metadata from structured fields -> Slack app action: Send Channel Message or Send Direct Message -> Microsoft Teams app action or Microsoft 365 companion action -> Gmail, Outlook, or Email by Zapier action. Notes: Use this secondary Zap when you need reusable delivery fan-out, fallback paths, or approval boundaries without falling back to Webhooks or Code steps.
