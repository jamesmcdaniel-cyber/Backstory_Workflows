# Channel Pulse — Workato Implementation Guide
...
8. Build the primary recipe `18_channel_pulse_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Channel Pulse trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Channel Pulse business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, CRM connector action, Meeting-system connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Channel Pulse, especially Identify Active Accounts, Gather Account Context. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Channel Pulse. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Channel Pulse. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Identify Active Accounts
3. Use Backstory custom connector action for Gather Account Context
4. Use an LLM connector or AI step to return strict JSON for AI Summarization
5. Use Slack connector for Route to Channel
6. Use Gmail, Outlook, or SMTP connector for Deliver Update
