# Customer Stack Blueprint — Workato Implementation Guide
...
- Delivery targets: Slack channel or direct message, Microsoft Teams channel or chat, Email delivery, Jira issue or project item, Asana task, Linear issue, Webhook, queue, database, or downstream workflow sink
...
8. Build the primary recipe `19_customer_stack_blueprint_primary_recipe` with the trigger surface Native CRM event trigger or API Platform endpoint.
...
- `normalize_run_context`: Normalize the Customer Stack Blueprint trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Customer Stack Blueprint business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, CRM connector action, Meeting-system connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Customer Stack Blueprint, especially Normalize Customer Context, Match Validated Patterns. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Customer Stack Blueprint. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Customer Stack Blueprint. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use CRM connector action for Normalize Customer Context
3. Use CRM connector action for Match Validated Patterns
4. Use an LLM connector or AI step to return strict JSON for AI Blueprint Design
5. Use database, queue, webhook, or downstream app connector for Deliver Blueprint
...
- `ai_blueprint_design`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent recommends the best starting asset, orchestration approach, connector substitutions, rollout milestones, and productization notes.
