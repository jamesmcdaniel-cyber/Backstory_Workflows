# Meeting Intelligence Normalizer — Workato Implementation Guide
...
8. Build the primary recipe `21_meeting_intelligence_normalizer_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Meeting Intelligence Normalizer trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Meeting Intelligence Normalizer business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Meeting-system connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Meeting Intelligence Normalizer, especially Fetch Source Payloads, Normalize Meeting Schema, Resolve Account Association. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Meeting Intelligence Normalizer. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Meeting Intelligence Normalizer. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use calendar connector action for Fetch Source Payloads
3. Use meeting-system connector action for Normalize Meeting Schema
4. Use meeting-system connector action for Resolve Account Association
5. Use an LLM connector or AI step to return strict JSON for AI Transcript QA
6. Use native connector for Publish Canonical Meeting Event
