# Adapter Regression Monitor — Workato Implementation Guide
...
- Source trigger in this catalog: Webhook / Schedule |
- Recommended Workato trigger surface: Scheduler by Workato
- Delivery targets: No end-user delivery surface; publish to a sink or downstream workflow.
...
8. Build the primary recipe `27_adapter_regression_monitor_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Adapter Regression Monitor trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Adapter Regression Monitor business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Meeting-system connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Adapter Regression Monitor, especially Load Golden Scenarios, Replay Adapter Cases. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Adapter Regression Monitor. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Adapter Regression Monitor. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use native connector action for Load Golden Scenarios
3. Use native connector action for Replay Adapter Cases
4. Use an LLM connector or AI step to return strict JSON for AI Regression Analysis
5. Use native connector for Deliver QA Findings
...
- `ai_regression_analysis`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: AI Agent summarizes behavior drift, root-cause hypotheses, and which workflows are likely impacted.
