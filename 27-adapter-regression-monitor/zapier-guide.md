# Adapter Regression Monitor — Zapier Implementation Guide
...
- `27_adapter_regression_monitor_primary_zap` — Trigger: Schedule by Zapier. Native steps: Native app step: Load Golden Scenarios -> Native app step: Replay Adapter Cases -> AI step backed by a native LLM app returning strict JSON for AI Regression Analysis -> Native delivery step: native app action for Deliver QA Findings. Notes: Keep the primary Zap focused on normalization, source access, and structured synthesis. Route secondary fan-out through additional native Zaps when template restrictions or retry requirements demand decomposition.
- `27_adapter_regression_monitor_delivery_or_followup_zap` — Trigger: Zapier Tables state change, Interfaces action, or an upstream custom app event. Native steps: . Notes: Use this secondary Zap when you need reusable delivery fan-out, fallback paths, or approval boundaries without falling back to Webhooks or Code steps.
