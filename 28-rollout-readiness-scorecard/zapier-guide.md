# Rollout Readiness Scorecard — Zapier Implementation Guide
...
- `28_rollout_readiness_scorecard_primary_zap` — Trigger: Private custom Zapier app trigger or internal Catch Hook rollout path. Native steps: Native app step: Normalize Deployment Prerequisites -> Native app step: Score Readiness Dimensions -> AI step backed by a native LLM app returning strict JSON for AI Launch Recommendation -> Native delivery step: Microsoft Teams app action for Deliver Readiness Scorecard. Notes: Keep the primary Zap focused on normalization, source access, and structured synthesis. Route secondary fan-out through additional native Zaps when template restrictions or retry requirements demand decomposition.
- `28_rollout_readiness_scorecard_delivery_or_followup_zap` — Trigger: Zapier Tables state change, Interfaces action, or an upstream custom app event. Native steps: Read delivery_payload and target metadata from structured fields -> Microsoft Teams app action or Microsoft 365 companion action. Notes: Use this secondary Zap when you need reusable delivery fan-out, fallback paths, or approval boundaries without falling back to Webhooks or Code steps.
