# Digital Chief of Staff — Workato Implementation Guide
...
- Source trigger in this catalog: Schedule or Slash Command
- Recommended Workato trigger surface: Scheduler by Workato
- Delivery targets: Slack channel or direct message, Calendar event or task
...
8. Build the primary recipe `29_digital_chief_of_staff_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Digital Chief of Staff trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Digital Chief of Staff business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, Meeting-system connector action.
- `assemble_business_context`: Join the intermediate context needed for Digital Chief of Staff, especially Source Adapter, Deterministic Routing. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Digital Chief of Staff. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Digital Chief of Staff. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Job report metadata.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Source Adapter
3. Use an LLM connector or AI step to return strict JSON for Account Update Synthesis
4. Use Backstory custom connector action for Deterministic Routing
5. Use Slack connector for Native Delivery
6. Use Google Calendar or Microsoft 365 Calendar connector for Calendar Writer
7. Use Slack connector for Run Summary
8. Record deterministic run summary and retry state
...
- `account_update_synthesis`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: Uses agent + MCP only for enrichment and account-summary synthesis.
