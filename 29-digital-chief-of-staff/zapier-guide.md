# Digital Chief of Staff — Zapier Implementation Guide
...
- `29_digital_chief_of_staff_primary_zap` — Trigger: Schedule by Zapier. Native steps: Backstory custom Zapier app action: source_adapter -> AI step backed by a native LLM app returning strict JSON for Account Update Synthesis -> Backstory custom Zapier app action: deterministic_routing -> Native delivery step: Slack app action for Native Delivery -> Native delivery step: Google Calendar or Microsoft Outlook Calendar action for Calendar Writer -> Native delivery step: Slack app action for Run Summary. Notes: Keep the primary Zap focused on normalization, source access, and structured synthesis. Route secondary fan-out through additional native Zaps when template restrictions or retry requirements demand decomposition.
- `29_digital_chief_of_staff_delivery_or_followup_zap` — Trigger: Zapier Tables state change, Interfaces action, or an upstream custom app event. Native steps: Read delivery_payload and target metadata from structured fields -> Slack app action: Send Channel Message or Send Direct Message -> Google Calendar or Microsoft Outlook Calendar action. Notes: Use this secondary Zap when you need reusable delivery fan-out, fallback paths, or approval boundaries without falling back to Webhooks or Code steps.
