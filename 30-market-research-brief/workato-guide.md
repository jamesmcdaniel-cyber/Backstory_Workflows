# Market Research Brief — Workato Implementation Guide
...
7. Build the primary recipe `30_market_research_brief_primary_recipe` with the trigger surface Scheduler by Workato.
...
- `normalize_run_context`: Normalize the Market Research Brief trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Market Research Brief business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action.
- `assemble_business_context`: Join the intermediate context needed for Market Research Brief, especially Load Research Packets, Add Backstory Context. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Market Research Brief. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Load Research Packets
3. Use Backstory custom connector action for Add Backstory Context
4. Use an LLM connector or AI step to return strict JSON for AI Market Synthesis
5. Use Gmail, Outlook, or SMTP connector for Deliver Weekly Digest
