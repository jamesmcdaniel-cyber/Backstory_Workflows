# Deal Inspection (Slack /dealcheck) — Workato Implementation Guide
...
- Source trigger in this catalog: Slack slash command — /dealcheck |
- Recommended Workato trigger surface: Workbot command, API Platform endpoint, or intake form trigger
...
8. Build the primary recipe `31_deal_inspection_primary_recipe` with the trigger surface Workbot command, API Platform endpoint, or intake form trigger.
...
- `normalize_run_context`: Normalize the Deal Inspection (Slack /dealcheck) trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Deal Inspection (Slack /dealcheck) business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Deal Inspection (Slack /dealcheck), especially Resolve Deal Context, Pull Backstory Signals. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Deal Inspection (Slack /dealcheck). Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Deal Inspection (Slack /dealcheck). Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Resolve Deal Context
3. Use Backstory custom connector action for Pull Backstory Signals
4. Use an LLM connector or AI step to return strict JSON for Generate Inspection Summary
5. Use Slack connector for Post Slack Result
...
- `generate_inspection_summary`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: Uses one agent pass to turn the merged signals into a concise risk summary and next steps.
