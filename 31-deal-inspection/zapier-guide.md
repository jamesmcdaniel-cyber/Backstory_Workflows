# Deal Inspection (Slack /dealcheck) — Zapier Implementation Guide
...
- Recommended Zapier trigger surface: Zapier Interfaces form/button, native app trigger, or private custom app trigger
...
- `31_deal_inspection_primary_zap` — Trigger: Zapier Interfaces form/button, native app trigger, or private custom app trigger. Native steps: Backstory custom Zapier app action: resolve_deal_context -> Backstory custom Zapier app action: pull_backstory_signals -> AI step backed by a native LLM app returning strict JSON for Generate Inspection Summary -> Native delivery step: Slack app action for Post Slack Result. Notes: Keep the primary Zap focused on normalization, source access, and structured synthesis. Route secondary fan-out through additional native Zaps when template restrictions or retry requirements demand decomposition.
- `31_deal_inspection_delivery_or_followup_zap` — Trigger: Zapier Tables state change, Interfaces action, or an upstream custom app event. Native steps: Read delivery_payload and target metadata from structured fields -> Slack app action: Send Channel Message or Send Direct Message. Notes: Use this secondary Zap when you need reusable delivery fan-out, fallback paths, or approval boundaries without falling back to Webhooks or Code steps.
