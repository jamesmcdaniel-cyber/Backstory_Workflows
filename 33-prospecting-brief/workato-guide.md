# Prospecting Brief — Workato Implementation Guide
...
- Source trigger in this catalog: Slack command / Webhook — on-demand brief |
...
8. Build the primary recipe `33_prospecting_brief_primary_recipe` with the trigger surface API Platform endpoint or native app event trigger.
...
- `normalize_run_context`: Normalize the Prospecting Brief trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Prospecting Brief business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action, Lookup tables or data tables.
- `assemble_business_context`: Join the intermediate context needed for Prospecting Brief, especially Resolve Account, Gather Prospecting Context. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Data tables.
- `render_delivery_payload`: Render normalized delivery_payload objects for Prospecting Brief. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.
- `record_run_summary`: Capture deterministic summary, dedupe, and retry state for Prospecting Brief. Inputs: run_context, delivery results. Outputs: execution summary. Native features: Variables, Data tables.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Resolve Account
3. Use Backstory custom connector action for Gather Prospecting Context
4. Use an LLM connector or AI step to return strict JSON for Generate Outreach Brief
5. Use Slack connector for Deliver Brief
...
- `generate_outreach_brief`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: Synthesizes the strongest outreach angle, why it matters now, and what to say next.
