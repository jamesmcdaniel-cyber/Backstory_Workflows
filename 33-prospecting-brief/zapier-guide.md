# Prospecting Brief — Zapier Implementation Guide
...
- `33_prospecting_brief_primary_zap` — Trigger: Private custom Zapier app trigger or internal Catch Hook rollout path. Native steps: Backstory custom Zapier app action: resolve_account -> Backstory custom Zapier app action: gather_prospecting_context -> AI step backed by a native LLM app returning strict JSON for Generate Outreach Brief -> Native delivery step: Slack app action for Deliver Brief. Notes: Keep the primary Zap focused on normalization, source access, and structured synthesis. Route secondary fan-out through additional native Zaps when template restrictions or retry requirements demand decomposition.
- `33_prospecting_brief_delivery_or_followup_zap` — Trigger: Zapier Tables state change, Interfaces action, or an upstream custom app event. Native steps: Read delivery_payload and target metadata from structured fields -> Slack app action: Send Channel Message or Send Direct Message. Notes: Use this secondary Zap when you need reusable delivery fan-out, fallback paths, or approval boundaries without falling back to Webhooks or Code steps.
