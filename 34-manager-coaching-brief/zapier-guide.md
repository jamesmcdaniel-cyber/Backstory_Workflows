# Manager Coaching Brief — Zapier Implementation Guide
...
- `34_manager_coaching_brief_primary_zap` — Trigger: Private custom Zapier app trigger or internal Catch Hook rollout path. Native steps: Backstory custom Zapier app action: resolve_opportunity -> Backstory custom Zapier app action: collect_coaching_signals -> AI step backed by a native LLM app returning strict JSON for Generate Coaching Notes -> Native delivery step: Slack app action for Post Coaching Brief. Notes: This Zap stays native-first and should rely on published app actions rather than ad hoc API requests.
