# Content Generation — Grounded Follow-Up — Workato Implementation Guide
...
- Source trigger in this catalog: Slack command / Webhook — follow-up generation |
...
7. Build the primary recipe `35_grounded_follow_up_primary_recipe` with the trigger surface API Platform endpoint or native app event trigger.
...
- `normalize_run_context`: Normalize the Content Generation — Grounded Follow-Up trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Content Generation — Grounded Follow-Up business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action.
- `assemble_business_context`: Join the intermediate context needed for Content Generation — Grounded Follow-Up, especially Resolve Opportunity Context, Pull Grounding Evidence. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Content Generation — Grounded Follow-Up. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Resolve Opportunity Context
3. Use Backstory custom connector action for Pull Grounding Evidence
4. Use an LLM connector or AI step to return strict JSON for Generate Follow-Up Draft
5. Use Slack connector for Deliver Draft
...
- `generate_follow_up_draft`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: Uses the model to draft the follow-up while grounding claims in the retrieved evidence.
