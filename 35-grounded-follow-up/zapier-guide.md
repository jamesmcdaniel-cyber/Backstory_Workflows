# Content Generation — Grounded Follow-Up — Zapier Implementation Guide
...
- `35_grounded_follow_up_primary_zap` — Trigger: Private custom Zapier app trigger or internal Catch Hook rollout path. Native steps: Backstory custom Zapier app action: resolve_opportunity_context -> Backstory custom Zapier app action: pull_grounding_evidence -> AI step backed by a native LLM app returning strict JSON for Generate Follow-Up Draft -> Native delivery step: Slack app action for Deliver Draft. Notes: This Zap stays native-first and should rely on published app actions rather than ad hoc API requests.
