# Pipeline & Forecast Digest — Workato Implementation Guide
...
7. Build the primary recipe `36_pipeline_forecast_digest_primary_recipe` with the trigger surface API Platform endpoint or native app event trigger.
...
- `normalize_run_context`: Normalize the Pipeline & Forecast Digest trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Pipeline & Forecast Digest business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action.
- `assemble_business_context`: Join the intermediate context needed for Pipeline & Forecast Digest, especially Pull Top Records, Enrich Each Opportunity, Aggregate Digest Inputs. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Pipeline & Forecast Digest. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Pull Top Records
3. Use Backstory custom connector action for Enrich Each Opportunity
4. Use Backstory custom connector action for Aggregate Digest Inputs
5. Use an LLM connector or AI step to return strict JSON for Generate Forecast Digest
6. Use Slack connector for Post Digest
