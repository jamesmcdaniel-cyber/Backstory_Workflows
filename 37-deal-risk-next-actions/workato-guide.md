# AI Agents — Deal Risk + Next Actions — Workato Implementation Guide
...
- Source trigger in this catalog: Slack command / Webhook — deal risk request |
...
7. Build the primary recipe `37_deal_risk_next_actions_primary_recipe` with the trigger surface API Platform endpoint or native app event trigger.
...
- `normalize_run_context`: Normalize the AI Agents — Deal Risk + Next Actions trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary AI Agents — Deal Risk + Next Actions business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action.
- `assemble_business_context`: Join the intermediate context needed for AI Agents — Deal Risk + Next Actions, especially Resolve Opportunity, Collect Deal Signals. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for AI Agents — Deal Risk + Next Actions. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Resolve Opportunity
3. Use Backstory custom connector action for Collect Deal Signals
4. Use an LLM connector or AI step to return strict JSON for Generate Risk Brief
5. Use Slack connector for Deliver Analysis
...
- `generate_risk_brief`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: Uses one model pass to produce the risk summary and next-action recommendation.
