# AI Agents — Deal Risk + Next Actions — Zapier Implementation Guide
...
- `37_deal_risk_next_actions_primary_zap` — Trigger: Private custom Zapier app trigger or internal Catch Hook rollout path. Native steps: Backstory custom Zapier app action: resolve_opportunity -> Backstory custom Zapier app action: collect_deal_signals -> AI step backed by a native LLM app returning strict JSON for Generate Risk Brief -> Native delivery step: Slack app action for Deliver Analysis. Notes: This Zap stays native-first and should rely on published app actions rather than ad hoc API requests.
