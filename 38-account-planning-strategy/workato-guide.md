# Account Planning & Strategy — Workato Implementation Guide
...
- Source trigger in this catalog: Slack command / Webhook — strategy brief request |
...
7. Build the primary recipe `38_account_planning_strategy_primary_recipe` with the trigger surface API Platform endpoint or native app event trigger.
...
- `normalize_run_context`: Normalize the Account Planning & Strategy trigger into the shared run_context contract. Inputs: trigger_payload, workflow defaults. Outputs: run_context. Native features: Recipe function by Workato, Formula mode, Variables.
- `load_source_records`: Load and normalize the primary Account Planning & Strategy business inputs into source_record objects. Inputs: run_context. Outputs: source_record[]. Native features: Recipe function by Workato, Backstory custom connector action.
- `assemble_business_context`: Join the intermediate context needed for Account Planning & Strategy, especially Resolve Account, Gather Account Signals. Inputs: source_record[], run_context. Outputs: source_record[], enrichment_context candidates. Native features: Lists, Object actions, Recipe data pills.
- `render_delivery_payload`: Render normalized delivery_payload objects for Account Planning & Strategy. Inputs: structured_ai_json, source_record, run_context. Outputs: delivery_payload. Native features: Recipe function by Workato, Object construction, Formula mode.

## Primary Recipe Outline

1. Call normalize_run_context
2. Use Backstory custom connector action for Resolve Account
3. Use Backstory custom connector action for Gather Account Signals
4. Use an LLM connector or AI step to return strict JSON for Generate Strategy Brief
5. Use Slack connector for Deliver Strategy Brief
...
- `generate_strategy_brief`: return JSON only with keys `title`, `summary`, `confidence`, `recommended_actions`, `source_refs`. Intent: Synthesizes priorities, risks, stakeholder guidance, and next moves for the account team.
