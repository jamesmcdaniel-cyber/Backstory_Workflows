# Account Planning & Strategy — Zapier Implementation Guide
...
- `38_account_planning_strategy_primary_zap` — Trigger: Private custom Zapier app trigger or internal Catch Hook rollout path. Native steps: Backstory custom Zapier app action: resolve_account -> Backstory custom Zapier app action: gather_account_signals -> AI step backed by a native LLM app returning strict JSON for Generate Strategy Brief -> Native delivery step: Slack app action for Deliver Strategy Brief. Notes: This Zap stays native-first and should rely on published app actions rather than ad hoc API requests.
